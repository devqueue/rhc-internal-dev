...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const fetchMailInbox = async (token) => {
      try {
        const response = await fetch(
          "https://graph.microsoft.com/v1.0/me/mailFolders/inbox/messages?$orderby=receivedDateTime DESC&$filter=receivedDateTime ge 2016-01-01T00:00:00Z and from/emailAddress/address eq 'mcenter@riyadhholding.sa'",
          {
            headers: { Authorization: "Bearer " + token },
          }
        );
