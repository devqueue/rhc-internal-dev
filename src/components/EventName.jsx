...
const EventName = ({ events }) => {
  ...
  const getNearestEvent = (events) => {
    const now = new Date();
    const upcomingEvents = events.filter(event => new Date(`${event.month} ${event.day}, 2024 ${event.starttime}`) > now);
    const sortedEvents = upcomingEvents.sort(
      (a, b) => new Date(`${a.month} ${a.day}, 2024 ${a.starttime}`) - new Date(`${b.month} ${b.day}, 2024 ${b.starttime}`)
    );
  ...
  useEffect(() => {
    if (events.length > 0) {
      ...
      const eventDate = new Date(`${nearestEvent.month} ${nearestEvent.day}, 2024 ${nearestEvent.starttime}`);
  ...
  return (
    <div className="py-[30px] px-[20px] bg-[#3B729C] flex flex-col justify-center items-center gap-[24px] text-white rounded-[8px]">
      <div className="flex flex-col items-center gap-[10px]">
        <h1 className="text-[20px] font-light ">{nearestEvent ? nearestEvent.name : 'No Event'}</h1>
