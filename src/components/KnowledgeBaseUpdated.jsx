...
const KnowledgeBaseUpdated = ({ Knowledge }) => {
  const [videoPopUp, setVideoPopUp] = useState(false);
  const [currentImg, setCurrentImg] = useState("");
  const handleVideo = (img) => {
    setCurrentImg(img);
  ...
  return (
    <div className="w-full min-h-[424px] bg-white rounded-lg overflow-hidden shadow-md relative">
      <div className="bg-[#3B729C] w-full h-[64px] flex justify-between items-center px-[30px] py-[20px] text-[white]">
        <h1 className="sm:text-[20px] text-[12px]">Awareness</h1>
        {/* <Link
          to="/all-employees"
          ...
          {Knowledge.map((item, index) => (
            <div className="min-h-[61px] lg:h-[61px] flex justify-between gap-[10px] self-stretch rounded-lg border-[0.5px] border-[#595959] py-[12px] px-[20px]">
              <div className="flex gap-[20px] items-center">
                ...
                <img src="/images/awareness.png" height="30" width="30" />
                <div className="column">
                  <h2 className="text-[#1E1E1E] text-[16px] font-semibold">
                    {item.fields.Name_en}
                  </h2>
                  <h2 className="text-[#1E1E1E] text-[12px] font-semibold opacity-50">
                    {item.fields.date}
                  </h2>
                </div>
              </div>
              ...
              <button
                ...
                onClick={() => {
                  handleVideo(item.fields.image_name);
                }}
              >
                view
              </button>
      ...
      {videoPopUp && (
        <div className="flex justify-center items-center w-full h-full bg-white absolute top-0 z-30">
          <img
            ...
            src={`https://riyadhholding.sharepoint.com/sites/Shamil/Assets/${currentImg}`}
            alt="img"
          />
          <div
